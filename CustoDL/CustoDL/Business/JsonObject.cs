...
namespace CustoDL.Business
{
    ...
    public class JsonObject : DynamicObject, IEnumerable, IEnumerator
    {
        ...
        public static dynamic GetDynamicJsonObject(string json)
        {
            ...
            return new CustoDL.Business.JsonObject(o);
        }

        internal static dynamic GetDynamicJsonObject(JObject jObj)
        {
            return new CustoDL.Business.JsonObject(jObj);
        }
...
namespace System
{
    ...
    {
        public static CustoDL.Business.JsonObject GetDynamicJsonObject(this Uri uri)
        {
            using (Net.WebClient wc = new Net.WebClient())
            {
                ...
                return CustoDL.Business.JsonObject.GetDynamicJsonObject(wc.DownloadString(uri.ToString()));
            }
        }

        public static string GetDynamicGeoCodeJson(this Uri uri)
        {
            using (Net.WebClient wc = new Net.WebClient())
            {
                wc.Encoding = System.Text.Encoding.UTF8;
                wc.Headers["User-Agent"] = "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 2.0.50727; .NET4.0C; .NET4.0E)";
                return wc.DownloadString(uri.ToString());
            }
        }
    }
}